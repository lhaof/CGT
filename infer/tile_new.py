...
def _post_process_patches(
    post_proc_func, post_proc_kwargs, pred_map, image_info, type_colour,
):
    ...
    #src_image = cv2.resize(src_image, (256,256), cv2.INTER_CUBIC)
    #print(inst_map.shape,inst_map.max())
    #inst_map = Image.fromarray(inst_map)
    #inst_map = inst_map.resize((256,256), resample=Image.NEAREST)
    #pred_map = Image.fromarray(pred_map)
    #pred_map = pred_map.resize((256,256), resample=Image.NEAREST)
    ...
    pred_inst, inst_info_dict = process(pred_map,inst_map, **post_proc_kwargs)
    #print(np.min(inst_info_dict))
    ...
    def process_file_list(self, run_args):
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                if 'PanNuke' + os.path.basename(file_path)[3:-4] + '.mat' in mat_files:
                    inst_path = self.inst_dir + 'PanNuke' + os.path.basename(file_path)[3:-4] + '.mat'
            ...
            for file_idx, file_path in enumerate(use_path_list):
                ...
                name, pred_map, pred_inst, inst_info_dict, overlaid_img = _post_process_patches(self.post_proc_func, post_proc_kwargs, type_map, file_info, self.type_info_dict)
