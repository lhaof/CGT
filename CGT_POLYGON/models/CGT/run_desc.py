...
def train_step(batch_data, run_info):
    ...
    for i in range(inst_map.shape[0]):
       ...
       inst_class,centers,edge_points,edge_index,pos_emd = get_bboxes(single_map,single_type_map)
    ...
    for branch_name in pred_dict.keys():
        ...
        class_loss = focal_loss(pred_classes,true_inst_classes_onehot)
...
####
def infer_step(batch_data, model,mode='train'):

    ####
    patch_imgs = batch_data['img']
    inst_map = batch_data["inst_map"]
    #print(patch_imgs.shape,inst_map.shape)
    ...
    batch_length = []
    batch_select_shape_feats = []
    if len(np.unique(inst_map)[1:]) > 0:
        for i in range(inst_map.shape[0]):
            single_map = inst_map[i,:,:].reshape(1,inst_map.shape[1],inst_map.shape[2])
            #true_bboxes,centers,edge_points,edge_index,pos_emd,select_shape_feats = get_infer_bboxes(single_map)
            centers,edge_points,edge_index,pos_emd = get_infer_bboxes(single_map)
            #batch_select_shape_feats.append(select_shape_feats)
            #batch_bboxes.append(true_bboxes)
            batch_length.append(centers.shape[0])
            batch_centers.append(centers)
            batch_edge_points.append(edge_points)
            batch_edge_indexs.append(edge_index)
            ...
            # --------------------------------------------------------------
            with torch.no_grad():  # dont compute gradient
            #pred_dict = model(patch_imgs_gpu,batch_bboxes,batch_centers,batch_edge_points,batch_edge_indexs,batch_pos_emb,batch_length,batch_select_shape_feats,mode='train')
                pred_dict = model(patch_imgs_gpu,batch_centers,batch_edge_points,batch_edge_indexs,batch_pos_emb,batch_length,mode='train')
                pred_map, pred_classes = pred_dict["tp"][0],pred_dict["tp"][1]
                pred_map = pred_map.permute(0, 2, 3, 1).contiguous()
        ...
        prediction = np.zeros((inst_map.shape[1],inst_map.shape[2]))
    output = add_class(inst_map,prediction)
    #output = output.reshape(inst_map.shape[0],inst_map.shape[1],inst_map.shape[2])
    # * Its up to user to define the protocol to process the raw output per step!
    return output
