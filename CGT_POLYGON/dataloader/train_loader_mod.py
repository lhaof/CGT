...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.image_path = file_path + 'images/'
        self.label_path = file_path + 'mat/'
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.cvtColor(cv2.imread(self.image_files[idx]), cv2.COLOR_BGR2RGB).astype("uint8") 
        ann_inst = sio.loadmat(self.label_files[idx])["inst_map"]
        ann_inst = ann_inst.astype("int32")
        ann_type = sio.loadmat(self.label_files[idx])["type_map"]
        ann = np.dstack([ann_inst, ann_type])
        ann = ann.astype("int32")
        centroid = sio.loadmat(self.label_files[idx])["inst_centroid"]
        #data = np.load(path)
