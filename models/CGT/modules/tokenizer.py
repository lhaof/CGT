...
class GraphFeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        hidden_dim = 256
        #print('num_atoms.dtype: ',type(num_atoms),type(hidden_dim))
        ...
        self.atom_encoder = nn.Linear(128, 256)
        self.edge_encoder = nn.Linear(64, 256)
    ...
    def get_batch(node_feature, edge_index, edge_feature, node_num, edge_num, perturb=None):
        ...
        #print(padded_node_mask.shape,padded_edge_mask.shape)
        padded_index = torch.zeros(b, max_len, 2, device=device, dtype=torch.long)  # [B, T, 2]
        #print(padded_index.shape,node_index.shape,edge_index.shape)
    ...
    def get_index_embed(node_id, node_mask, padded_index):
        ...
        d = node_id.size(-1)
        node_id = node_id.to('cuda:0')
        padded_node_id = torch.zeros(b, max_n, d, device=node_id.device, dtype=node_id.dtype).to('cuda:0')  # [B, max_n, D]
        padded_node_id[node_mask.to('cuda:0')] = node_id

        padded_node_id = padded_node_id[:, :, None, :].expand(b, max_n, 2, d)
        padded_index = padded_index[..., None].expand(b, max_len, 2, d).to('cuda:0')
    ...
    def forward(self, batched_data, perturb=None):
        ...
        #print(node_data.shape,edge_data.shape,batched_data["in_degree"].shape,batched_data["out_degree"].shape)
        ...
        if self.lap_node_id:
            ...
            #print('lap_node_id',eigvec)
            lap_node_id = self.handle_eigvec(eigvec, node_mask, self.lap_node_id_sign_flip)
            #print('lap_node_id',lap_node_id)
            #print('padded_index',padded_index)
            
            lap_index_embed = self.get_index_embed(lap_node_id, node_mask, padded_index)  # [B, T, 2Dl]
            #print(lap_index_embed.shape)
            #a = lap_index_embed[0][3]
            #for i in range(lap_index_embed.shape[1]):
                #b = lap_index_embed[0][i]
                #print(b.shape,a.shape,torch.dot(a,b.T))
            #print(padded_feature,self.lap_encoder(lap_index_embed).shape)
            #padded_feature = padded_feature + self.lap_encoder(lap_index_embed)
            #print(padded_feature.shape,self.lap_encoder(lap_index_embed).shape)
            padded_feature = torch.cat((padded_feature,self.lap_encoder(lap_index_embed)),2)
        #print(padded_feature.shape)
        #print(self.get_type_embed(padded_index).shape,self.lap_encoder(lap_index_embed).shape)
        if self.type_id:
            #padded_feature = padded_feature + self.get_type_embed(padded_index)
            #print(self.get_type_embed(padded_index).shape)
            padded_feature = torch.cat((padded_feature,self.get_type_embed(padded_index)),2)
        #padded_feature = torch.cat((padded_feature_lap,padded_feature_type),2)
        #print(padded_feature.shape,self.get_type_embed(padded_index).shape)
