...
def get_infer_bboxes(inst_map,edge_num):
...
def get_bboxes(inst_map,type_map,edge_num):
...
def add_class(inst_map,classes):
    #print(inst_map.shape,classes.shape)
    obj_ids = torch.unique(inst_map)
...
class_weights = torch.tensor([5,2,4]).to("cuda")
...
def focal_loss(outputs, targets,class_weights,alpha=1, gamma=2,logits=False, reduce=False):
