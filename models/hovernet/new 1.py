...
from .utils import crop_to_shape, dice_loss, mse_loss, msge_loss, xentropy_loss, get_bboxes,focal_loss,weighted_entropy_loss
import os
...
def train_step(batch_data, run_info):
    ...
    for i in range(inst_map.shape[0]):
       ...
       true_bboxes,inst_class = get_bboxes(single_map,single_type_map)
    ...
    pred_dict = model(imgs,batch_bboxes,mode='train')
...
def valid_step(batch_data, run_info):
    ...
    with torch.no_grad():  # dont compute gradient
        pred_dict = model(imgs_gpu,batch_bboxes,mode='train')
        pred_map, pred_classes = pred_dict["tp"][0],pred_dict["tp"][1]
        pred_map = pred_map.permute(0, 2, 3, 1).contiguous()
...
def infer_step(batch_data, model,mode='test'):
    ...
    # --------------------------------------------------------------
    with torch.no_grad():  # dont compute gradient
        pred_dict = model(patch_imgs_gpu,batch_bboxes,mode)
        pred_dict = OrderedDict(
            [[k, v.permute(0, 2, 3, 1).contiguous()] for k, v in pred_dict.items()]
        )
